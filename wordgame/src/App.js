...
import { addPassCount } from './redux/scoring/scoringActions';
...
class App extends React.Component {
  render() {
    ...
    return (
      <div className="container">
        ...
        <MiniScores activePlayer={activePlayer} players={players} />
        <GameBoard
  ...
  boardClick = (e, index, isActive) => {
    ...
    if (clickedLetter.length === 0 && isActive)
      newClicked = [thisTile.stack[0], null, index];
    if (clickedLetter.length > 0 && !isActive) {
      if (clickedLetter[0] === thisTile.stack[0])
        return this.props.newErrMsg(
          `this letter is already ${thisTile.stack[0]}`
        );
    ...
    if (clickedLetter.length > 2 && isActive) {
      if (newBoard[activeIndex].stack[0] === thisTile.stack[1])
        return this.props.newErrMsg(
          `this letter is already ${thisTile.stack[1]}`
        );
      if (newBoard[activeIndex].stack[1] === thisTile.stack[0])
        return this.props.newErrMsg(
          `this letter is already ${thisTile.stack[0]}`
        );
    ...
    if (clickedLetter.length === 2 && isActive) {
      if (clickedLetter[0] === thisTile.stack[1])
        return this.props.newErrMsg(
          `this letter is already ${thisTile.stack[1]}`
        );
  ...
  dictionaryCheck = word => {
    if (!this.props.dictionary)
      return this.props.newMessage(`dictionary loading... please wait...`);
  ...
  passLetters = num => {
    ...
    if (emptyBag && players.length - 1 === passCount)
      return this.props.newErrMsg('END GAME // PASSCOUNT MAX');
    ...
    if (clickedLetter.length > 0) {
      ...
      if (activeIndex || activeIndex === 0) {
        ...
        if (thisTile.stack[1] === letter)
          return this.props.newErrMsg(`this letter is already ${letter}`);
  ...
  onDrop = (e, droppedOnIndex, onActive) => {
    ...
    if (droppedOnLetter === incomingLetter)
      return this.props.newErrMsg(`this letter is already ${incomingLetter}`);
    ...
    if (incomingIndex !== '') {
      if (
        ...
        return this.props.newErrMsg(
          `this letter is already ${droppedOnTile.stack[0]}`
        );
  ...
  passTurn = () => {
    ...
    if (activeTiles.length > 0)
      return this.props.newErrMsg('cannot pass with active tiles on board');
    else return this.nextPlayer(0);
  }; // this.passTurn() END >> this.nextPlayer(0);

  submitLetters = () => {
    const activeTiles = this.props.gameBoard
      .filter(tile => tile.active)
      .sort((a, b) => a.id - b.id);
    if (activeTiles.length < 1)
      return this.props.newErrMsg("you haven't placed any tiles");
  ...
  scoreWords = foundWords => {
    ...
    if (!okStrict)
      return this.props.newErrMsg('Error: Cannot build in both directions!');
...
const mapStateToProps = state => ({
  ...
  clickedLetter: state.letters.clickedLetter,
  dictionary: state.config.dictionary,
  emptyBag: state.letters.emptyBag,
  errMsg: state.commo.errMsg,
  gameBoard: state.config.gameBoard,
  gameHistory: state.commo.gameHistory,
  letterBag: { ...state.letters.letterBag },
  message: state.commo.message,
  passCount: state.scoring.passCount,
